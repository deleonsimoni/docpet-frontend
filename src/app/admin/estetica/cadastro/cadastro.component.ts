...
export class CadastroComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.esteticaForm = this.formBuilder.group(
      {
        _id: [null],
  ...
  popular(){
    this.esteticaService.get(this.id).subscribe(res => {

      let vet = res;

      this.esteticaForm.patchValue(vet);
      ...
      if(vet.estabelecimentos.length > 0){
        ...
        this.estabelecimentos = this.esteticaForm.get('estabelecimentos') as FormArray;

        for(var i=1; i < vet.estabelecimentos.length; i++){
          this.addEstabelecimento();
        }
        this.esteticaForm.get('estabelecimentos').patchValue(vet.estabelecimentos);
      }

      if(vet.formacoes && vet.formacoes.length > 0){

        this.formacoes = this.esteticaForm.get('formacoes') as FormArray;

        for(var i=1; i < vet.formacoes.length; i++){
          this.addFormacao();
        }

        this.esteticaForm.get('formacoes').patchValue(vet.formacoes);
      }

      if(vet.experiencias && vet.experiencias.length > 0){

        this.experiencias = this.esteticaForm.get('experiencias') as FormArray;

        for(var i=1; i < vet.experiencias.length; i++){
          this.addExperiencia();
        }

        this.esteticaForm.get('experiencias').patchValue(vet.experiencias);
      }

      if(vet.conquistas && vet.conquistas.length > 0){

        this.conquistas = this.esteticaForm.get('conquistas') as FormArray;

        for(var i=1; i < vet.conquistas.length; i++){
          this.addConquista();
        }
        this.esteticaForm.get('conquistas').patchValue(vet.conquistas);
      }
  ...
  salvar(){
    if (this.esteticaForm.valid) {
      const novaEstetica = this.esteticaForm.getRawValue();

      if(this.url){
        novaEstetica.img = this.url;
      }

      if(this.id){
        this.esteticaService.update(this.id, novaEstetica).subscribe(
          () => {
      ...
      this.esteticaService.create(novaEstetica).subscribe(
          () => {
            this.router.navigate(['/admin/list-estetica']);
          },
          (error) => {
  ...
  getFormValidationErrors() {
    Object.keys(this.esteticaForm.controls).forEach(key => {
      const controlErrors: ValidationErrors = this.esteticaForm.get(key).errors;
  ...
  buscarMap():void{
    ...
    this.cepService.getLocale(JSON.stringify(novaEstetica.endereco)).subscribe(data=>{
      this.lat = data.lat;
      this.lng = data.lng
      this.showMap = true;
    }, error=>{
  ...
  buscarCEP(cep:String):void{
    this.cepService.get(cep).subscribe(data=>{
      this.esteticaForm.get('endereco').patchValue({'bairro':data.bairro, 'logradouro': data.logradouro, 'municipio': data.localidade, 'estado' : data.uf});
    }, error=>{
      console.log(error);
    })
  }
